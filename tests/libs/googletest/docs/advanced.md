...
(one third of Boolean assertions in the Google code _base are negative ones), and
...
NOTE: `CHECK_DOUBLE_EQ()` in `_base/logging.h` uses a fixed absolute error bound,
...
```c++
#include "testing/_base/public/gmock.h"
...
in order to take advantage of this. Just include `"testing/_base/public/gmock.h"`
